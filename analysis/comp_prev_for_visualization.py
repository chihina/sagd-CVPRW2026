...
model_names_omit.append(r'MTGS-PP')

model_name = '09-12-36_combined_social_COA_False_bce_SOC_True_hm_coef_iter_temp_2-3_1_0.0001_FRZ_IT_VE'
model_names.append(model_name)
model_names_omit.append(r'MTGS-Soc.')

model_name = '11-39-06_gazelle_1e-06'
model_names.append(model_name)
model_names_omit.append(r'Gaze-LLE-PP')
...
for idx, result in enumerate(results):
    ...
    for model_name_omit in model_names_omit:
        if 'Ours' in model_name_omit:
            ...
            save_data_ious_result.append(group_iou_pp)
            save_data_dists_result.append(group_dist_pp)
        elif 'MTGS-Soc.' in model_name_omit:
            save_data_ious_result.append(group_iou_pairwise)
            save_data_dists_result.append(group_dist_pairwise)
        else:
            assert False, "Unknown model name omit"
...
iou_diff_threshold = 0.05
dist_diff_threshold = 20.0

# initialize all indices as selected
selected_indices = df_ious.index
if 'MTGS-PP' in df_ious.columns:
    selected_indices_pp = df_ious.index[(df_ious['Ours'] - df_ious['MTGS-PP']) > iou_diff_threshold]
    selected_indices = selected_indices.intersection(selected_indices_pp)
    print(f"Number of samples where Ours outperforms MTGS-PP by more than {iou_diff_threshold} IoU: {len(selected_indices_pp)}")
# if 'Gaze-LLE-PP' in df_ious.columns:
    # selected_indices_gaze = df_ious.index[(df_ious['Ours'] - df_ious['Gaze-LLE-PP']) > iou_diff_threshold]
    # selected_indices = selected_indices.intersection(selected_indices_gaze)
    # print(f"Number of samples where Ours outperforms Gaze-LLE-PP by more than {iou_diff_threshold} IoU: {len(selected_indices_gaze)}")
# if 'MTGS-Soc.' in df_ious.columns:
    # selected_indices_soc = df_ious.index[(df_ious['Ours'] - df_ious['MTGS-Soc.']) > iou_diff_threshold]
    # selected_indices = selected_indices.intersection(selected_indices_soc)
    # print(f"Number of samples where Ours outperforms MTGS-Soc. by more than {iou_diff_threshold} IoU: {len(selected_indices_soc)}")

print(f"Number of samples where Ours outperforms MTGS-PP by more than {iou_diff_threshold} IoU: {len(selected_indices)}")
...
if len(selected_indices) == 0:
    print("No samples found where Ours outperforms MTGS-PP by more than the specified IoU threshold.")
    selected_indices = df_ious.index[:5]  # select top 5 samples with highest IoU difference
...
for data_id in selected_indices:
    ...
    for model_name_omit in model_names_omit:
        ...
        if 'Ours' in model_name_omit:
            ...
            coatt_level_pred = result_sample['coatt_level_pred_grp_best_pp'][0]
            coatt_level_flag = torch.sum(coatt_level_pred > coatt_level_thresh, dim=-1) > 1
            coatt_hm_pred_pk_cd = result_sample['coatt_hm_pred_pp_pk_cd'][0][coatt_level_flag]
            coatt_level_pred = coatt_level_pred[coatt_level_flag]
        elif 'MTGS-Soc.' in model_name_omit:
            coatt_level_pred = result_sample['coatt_level_pred_label_pairs_best'][0]
            coatt_level_flag = torch.sum(coatt_level_pred > coatt_level_thresh, dim=-1) > 1
            coatt_hm_pred_pk_cd = result_sample['coatt_hm_pred_pairs_pk_cd'][0][coatt_level_flag]
            coatt_level_pred = coatt_level_pred[coatt_level_flag]
        ...
        for grp_idx, pk_cd in enumerate(coatt_hm_pred_pk_cd):
            ...
            if grp_member > 1:
                ...
                cv2.circle(img_vis, (pk_x, pk_y), sa_point_size, grp_color, -1)  # colored circle for predicted co-attention points
                cv2.circle(img_vis_only_bboxes, (pk_x, pk_y), sa_point_size, grp_color, -1)  # colored circle for predicted co-attention points
                ...
                for head_idx in range(coatt_level_pred_grp.shape[0]):
                    if coatt_level_pred_grp[head_idx] > coatt_level_thresh:
                        ...
                        cv2.rectangle(img_vis, (x1, y1), (x2, y2), grp_color, head_rect_size)  # colored box for heads with high co-attention level
                        cv2.rectangle(img_vis_only_bboxes, (x1, y1), (x2, y2), grp_color, head_rect_size)  # colored box for heads with high co-attention level
    ...
    for grp_idx, pk_cd in enumerate(coatt_hm_gt_pk_cd):
        ...
        cv2.circle(img_vis_gt, (pk_x, pk_y), sa_point_size, grp_color, -1)  # colored circle for ground-truth co-attention points
        ...
        for head_idx in range(coatt_level_gt_grp.shape[0]):
            if coatt_level_gt_grp[head_idx] == 1:
                ...
                # change color based on group index
                cv2.rectangle(img_vis_gt, (x1, y1), (x2, y2), grp_color, head_rect_size)  # colored box for heads with high co-attention level
