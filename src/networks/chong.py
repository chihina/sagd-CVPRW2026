...
class NoraNet(nn.Module):
    # Define a ResNet 50-ish arch
    def __init__(self, block=Bottleneck, layers_scene=[3, 4, 6, 3, 2]):
        # Resnet Feature Extractor
        self.inplanes_scene = 64
        self.inplanes_face = 64
        super().__init__()
    ...
    def forward(self, sample):
        ...
        im = torch.cat([image, gaze_map.unsqueeze(1)], dim=1)
        ...
        # Encoding scene features
        padding = torch.zeros_like(scene_feat)  # (N, 1024, 7, 7)
        scene_feat = torch.cat((scene_feat, padding), dim=1)  # (N, 2048, 7, 7)
