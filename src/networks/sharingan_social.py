...
from src.utils import generate_gaze_cone, pair, build_2d_sincos_posemb
...
# ==================================================================================================================== #
class Sharingan_social(nn.Module):
    def __init__(self,
                 output: str = "point", # single_person
                 patch_size: int = 16,
                 token_dim: int = 768,
                 image_size: int = 224,
        ...
        assert output in ["point"], f"Argument output value `{output}` is wrong."
        ...
        self.head_mask_embedder = HeadMaskEmbedder(
        ...
        if output == "point":
            ...
            # Init bias of last layer to the mean of the gaze annotations
            self.gaze_decoder.fc.bias.data = torch.tensor([0.5, 0.5])
            
            # Predict social gaze
            self.speaking_proj = nn.Linear(1, token_dim)
            self.head_bbox_proj = nn.Linear(4, token_dim)
            self.people_attention = GraphGaze(in_channels=768, hidden_channels=192, heads=4)
            self.decoder_lah = LinearDecoderSocialGraph(token_dim)    # decoder for looking at heads
            self.decoder_laeo = LinearDecoderSocialGraph(token_dim)    # decoder for laeo
            self.decoder_sa = LinearDecoderSocialGraph(token_dim)    # decoder for shared attention
#             self.decoder_social = LinearDecoderSocialMLP(token_dim)    # for MLP model

        self.inout_decoder = InOutClassifier(token_dim + token_dim)
    
    
    def forward(self, x):
        # x = {"image": img, "heads": head_imgs, "head_bboxes": head_bboxes, "head_mask": head_mask}
        
        # Predict Gaze Embed ===================================================
        b, n, c, h, w = x["heads"].squeeze(1).shape # n = total nb of people
        gaze_emb, _, gaze_token, gaze_vec = self.gaze_encoder(x["heads"].view(-1, c, h, w), x["head_bboxes"].view(-1, 4))        
        gaze_token = gaze_token.view(b, n, -1) # -1 corresponds to token_dim
        
        # Tokenize Inputs =====================================================
        image_tokens = self.image_tokenizer(x["image"].squeeze(1)) # (B, num_tokens, token_dim)
        ...
        if self.output == "point":
            input_tokens = torch.cat([
                image_tokens,
                gaze_token
            ], dim=1) # (B, num_tokens+nb_people, token_dim)
            
            # Encode
            output_tokens = self.encoder(input_tokens, return_all_layers=True) # (B, num_tokens+1+num_global_tokens, token_dim)
            
            # Decode 
            gaze_pt = self.gaze_decoder(output_tokens[-1][:, -n-1:-1, :].reshape(b * n, -1)) # decode only gaze tokens (last position is global token)
            
            # add head bbox and speaking status information
            people_tokens = output_tokens[-1][:, -n-1:-1, :].clone()   # B, N, token_dim
            speaking_emb = self.speaking_proj(x["speaking"].view(-1, 1))    # get speaking status emb
            head_bbox_emb = self.head_bbox_proj(x["head_bboxes"].view(-1, 4))    # get new head bbox emb
            people_tokens = people_tokens + speaking_emb.view(b,n,-1) + head_bbox_emb.view(b,n,-1)      # add head bbox, speaking status embedding
            
            # apply attention on person tokens
            people_tokens = self.people_attention(people_tokens, x['num_valid_people'].squeeze(1))    # for graph
            
            # make person pairs
            indices = torch.tensor(list(itertools.permutations(torch.arange(n), 2))).T # (2, num_pairs)
            num_pairs = indices.shape[1]
            
            output_person_tokens = people_tokens # (B, N, D)
            opt_1 = torch.cat([output_person_tokens[:, [i], :] for i in indices[0]], dim=1) # (B, num_pairs, D) - left terms
            opt_2 = torch.cat([output_person_tokens[:, [j], :] for j in indices[1]], dim=1) # (B, num_pairs, D) - right terms
            person_token_pairs = torch.cat([opt_1, opt_2], dim=2) # (B, num_pairs, 2*D)
            person_token_pairs = person_token_pairs.reshape(b * num_pairs, -1) # (B*num_pairs, 2*D)

            # predict social gaze
            lah = self.decoder_lah(person_token_pairs)
            laeo = self.decoder_laeo(person_token_pairs)
            coatt = self.decoder_sa(person_token_pairs)
            
            output_person_tokens = output_person_tokens.reshape(b * n, -1)
            input_person_tokens = gaze_token.view(b * n, -1)
            inout = self.inout_decoder(torch.cat([output_person_tokens, input_person_tokens], dim=1)) # (B*N, 2*token_dim) >> (B*N, 1)

            # include dimension for time
            return gaze_vec.view(b, n, -1).unsqueeze(1), gaze_pt.view(b, n, -1).unsqueeze(1), inout.view(b, n).unsqueeze(1), lah.view(b, num_pairs).unsqueeze(1), laeo.view(b, num_pairs).unsqueeze(1), coatt.view(b, num_pairs).unsqueeze(1)
...
class LinearDecoder(nn.Module):
    def __init__(self, token_dim):
        ...
        self.block1 = ResidualLinearBlock(2 * token_dim)
        self.block2 = ResidualLinearBlock(token_dim // 2)
...
class SimpleLinearDecoder(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.fc1 = nn.Linear(feature_dim, feature_dim // 4, bias=False)
        self.bn1 = nn.BatchNorm1d(feature_dim // 4)
        self.fc2 = nn.Linear(feature_dim // 4, feature_dim // 16, bias=False)
        self.bn2 = nn.BatchNorm1d(feature_dim // 16)
        self.fc3 = nn.Linear(feature_dim // 16, feature_dim // 64, bias=False)
        self.bn3 = nn.BatchNorm1d(feature_dim // 64)

        self.fc_out = nn.Linear(feature_dim // 64, 2)
...
class InOutClassifier(nn.Module):
...
class LinearDecoderSocialMLP(nn.Module):
    def __init__(self, token_dim):
        super().__init__()
        self.token_dim = token_dim
        
        scale = 2
        self.fc_pre = nn.Linear(2*token_dim, 2*token_dim)
        self.block1 = ResidualLinearBlock(2*token_dim, scale=scale)
        self.block2 = ResidualLinearBlock(2*token_dim // scale**2, scale=scale)
        self.fc_post = nn.Linear(2*token_dim // scale**4, 1)
        
    def forward(self, edge_tokens):
        
        x = self.fc_pre(edge_tokens)
        x = self.block1(x)
        x = self.block2(x)
...
class AttentionDecoderSocial(nn.Module):
    def __init__(self, 
                 token_dim, 
                 depth: int = 2,
                 num_heads: int = 12,
        ...
        self.mlp_block1 = ResidualLinearBlock(2 * token_dim)
        self.mlp_block2 = ResidualLinearBlock(token_dim // 2)
        self.fc = nn.Linear(token_dim // 8, 1)
        
    def forward(self, people_tokens):
        
        b, n, _ = people_tokens.shape
        
        # attention block (node updating)
        people_tokens = self.att_block(people_tokens)
        
        # mlp block (edge modelling)
        extra_people_tokens = people_tokens[:, :-1, :]    # B, n-1, token_dim
        target_person_token = people_tokens[:, -1, :].unsqueeze(1).repeat((1, n-1, 1))    # B, n-1, token_dim
        edge_tokens = torch.cat([extra_people_tokens, target_person_token], dim=-1)    # B, n-1, token_dim*2
        x = self.mlp_block1(edge_tokens.reshape(b*(n-1), -1))
        x = self.mlp_block2(x)
...
class GraphGaze(nn.Module):
    ...
    def forward(self, x, num_valid_people):
        ...
        x = x_orig + self.conv1(batch.x, batch.edge_index)
        x = F.relu(self.bn1(x))
        x = x_orig + self.conv2(x, batch.edge_index)
        x = F.relu(self.bn2(x))        
        x = self.readout(x)
...
#             edge_index_list.append(torch.combinations(torch.arange(num_people-num_valid_people[i], num_people, device=device), r=2).T)
            if num_valid_people[i]==1:
                edge_index_list.append(torch.combinations(torch.arange(num_people-num_valid_people[i], num_people, device=device), r=2).T) 
            else:
                edge_index_list.append(torch.tensor(list(itertools.permutations(torch.arange(num_people-num_valid_people[i], num_people, device=device), 2)), device=device).T)
...
class HeadMaskEmbedder(nn.Module):
    def __init__(self, patch_size=(16, 16), token_dim=768):
        super().__init__()
        
        self.patch_size = pair(patch_size)
        self.P_W, self.P_H = self.patch_size
        self.token_dim = token_dim
        
        self.embedder = nn.Embedding(self.P_H * self.P_W + 1, self.token_dim)
        
    def forward(self, head_mask):
        head_pos_emb = F.avg_pool2d(head_mask, 
                                    kernel_size=(self.P_H, self.P_W),
                                    stride=(self.P_H, self.P_W),
                                    divisor_override=1
                                   )
        head_pos_emb = einops.rearrange(head_pos_emb, "b c h w -> b (c h w)") # Bx1xHxW >> BxHW
        head_pos_emb = head_pos_emb.to(torch.long)
        head_pos_emb = self.embedder(head_pos_emb) # BxHW >> BxHWxD
...
class GazeEncoder(nn.Module):
    def __init__(self, token_dim=768, feature_dim=512):
        ...
        base = models.resnet18(pretrained=False)  # type: ignore
        ...
        self.proj = nn.Sequential( # to compute a gaze token from the gaze embedding + head position
            #nn.Linear(embed_dim + 4, token_dim),
            nn.Linear(embed_dim, token_dim),
    ...
    def forward(self, x_head, x_bbox):
        gaze_emb = self.backbone(x_head).flatten(-3, -1)  # (N, embed_dim), embed_dim=512
        
        #gaze_token = self.proj(torch.cat([gaze_emb, x_bbox], dim=1)) # (N, token_dim)
        head_bbox_emb = self.pos_proj(x_bbox)
        gaze_token = self.proj(gaze_emb) + head_bbox_emb
        gaze_vec = self.predictor(gaze_emb)
        gaze_vec = F.normalize(gaze_vec, p=2, dim=1) # normalize to unit vector
        
        return gaze_emb, head_bbox_emb, gaze_token, gaze_vec
