...
class VideoCoAttDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        coatt_ids = torch.from_numpy(img_annotations["coatt_id"].values.astype(int))
...
class VideoCoAttDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        if stage == "fit":
            ...
            self.train_dataset = VideoCoAttDataset(
                root=self.root, 
                split="train", 
                stride=3,
            ...
            self.val_dataset = VideoCoAttDataset(
                root=self.root, 
                split="val", 
                stride=3,
            ...
            self.test_dataset = VideoCoAttDataset(
                root=self.root, 
                split="test", 
                stride=3,
            ...
            self.predict_dataset = VideoCoAttDataset(
                root=self.root, 
                split="test",
                stride=1,
