...
def visualize_gaze(batch, social_preds=None):
...
def post_process_coatt_pred(gaze_pts_pred: list, inout_pred: list, dist_thr: float) -> list:
    coatt_level_pred = []
    inout_thr = 0.5
    gaze_pts_pred = gaze_pts_pred.unsqueeze(1)
    inout_pred = inout_pred.unsqueeze(1)
...
def visualize_coatt(frame, batch, plot_mode):
    h, w = frame.shape[:2]
    ...
    if plot_mode=='coatt':
        ...
        for token_idx in range(coatt_level.shape[0]):
            ...
            if coatt_level_exist:
                ...
                for person_idx in range(coatt_level_pred.shape[0]):
                    ...
                    fin = ((head_bbox[0]+head_bbox[2])//2, head_bbox[3] + 15)
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    fontScale = 1
                    thickness = 1
                    color_text = (255, 255, 255)
        ...
        head_bboxes = batch['head_bboxes'][0]
        # print('Head bboxes shape:', head_bboxes.shape)
        coatt_level_pred_pp = post_process_coatt_pred(batch['gp_pred'], batch['inout_pred'], dist_thr=0.1)
        coatt_level_pred_pp = coatt_level_pred_pp[0][0]

        for token_idx in range(coatt_level_pred_pp.shape[0]):
            coatt_level_pred = coatt_level_pred_pp[token_idx]
            coatt_level_exist = torch.sum(coatt_level_pred) > 1
            if coatt_level_exist:
                # print(f'Token {token_idx} PP:', coatt_level_pred)
                for person_idx in range(coatt_level_pred.shape[0]):
                    coatt_level_pred_person = coatt_level_pred[person_idx]
                    # print(f'Person {person_idx} coatt level PP:', coatt_level_pred_person)
                    head_bbox = head_bboxes[person_idx].clone()
                    if torch.sum(head_bbox) == 0:
                        continue
                    head_bbox = perc_to_pixel(head_bbox, h, w).int().cpu().numpy()
...
# main function
def compute(results, clip_name, plot_mode):
    ...
    base_h, base_w = frame_tmp.shape[:2]
    ...
    out = cv2.VideoWriter(save_path, fourcc, 8.0, (base_w, base_h))
    for idx in tqdm(sorted_indices):
        ...
        if 'lah_pred' in batch.keys() and 'laeo_pred' in batch.keys() and 'coatt_pred' in batch.keys():
            # LOAD LAH
        ...
        frame = visualize_coatt(frame, batch, plot_mode)
...
if __name__=='__main__':
    ...
    # model_dir = "combined_social/2026-02-10/18-29-09_combined_social_COA_True_bce_SOC_True_hm_coef_iter_temp_2-3_1_0.0001_FRZ_IT_VE"

    # mtgs-pp
    # model_dir = "combined_social/2026-02-11/09-12-36_combined_social_COA_False_bce_SOC_True_hm_coef_iter_temp_2-3_1_0.0001_FRZ_IT_VE"

    # ours
    # model_dir = "combined_social/2026-02-12/16-54-39_combined_social_COA_True_bce_SOC_True_hm_coef_iter_temp_2-3_1_0.0001_normal_FRZ_IT_VE_wo_det_heads"

    # ours
    model_dir = "combined_social/2026-02-13/17-23-01_combined_social_COA_True_True_bce_SOC_True_hm_coef_iter_temp_2-3_1_1e-05_w_gaze_vec_FRZ_IT_VE"

    # ours
    # model_dir = "combined_social/2026-02-14/11-00-29_combined_social_COA_True_False_bce_SOC_True_hm_coef_iter_temp_2-3_1_1e-05_w_gaze_vec_FRZ_GE_GEB_IT_VE_VA_GD_ID"

    if 'COA_True_' in model_dir:
        plot_mode = 'coatt' # {gaze, coatt}
    else:
        plot_mode = 'gaze' # {gaze, coatt}
    ...
    for clip_name in clip_name_list:
        compute(results, clip_name, plot_mode)